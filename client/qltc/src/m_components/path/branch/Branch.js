...
const Dish = () => {
  ...
  const deleteDish = async (e, id) => {
    ...
    try {
      const request = await authApi().delete(endpoints["dish-delete"](id), {
        method: "DELETE",
  ...
  const deactivateDish = async (e, id, isActive) => {
    e.preventDefault();
    const url = isActive
      ? endpoints["dish-deactivate"](id)
      : endpoints["dish-activate"](id);
    ...
    <>
      <div className="d-flex justify-content-end me-2">
        <div className="text-primary">
          <Link to="/admin/dish/create">
            <AiFillPlusCircle /> Create a new dish{" "}
          </Link>
        </div>
        <div>
          {" "}
                ...
                <td>
                  <div>
                    <Link
                      ...
                      to={"/admin/dish/view?id=" + dish.id}
                    >
                      <GrFormView /> View
                    </Link>
                    <Link
                      className="text-decoration-none"
                      to={"/admin/dish/edit?id=" + dish.id}
                    >
                      <AiFillEdit /> Edit
                    </Link>
                    <Link
                    ...
                    <Link
                      ...
                      too=""
                      onClick={(e) => {
                        deactivateDish(e, dish.id, dish.isAvailable);
                      }}
                    >
                      <BiSolidDish />
