...
const UpdateHallPrice = (props) => {
    ...
    const updateHallPrice = async (e, id) => {
        ...
        try {
            const api = await authApi().post(endpoints['hall-price-update'](id), data)
            setState({"hallPriceUpdate": "Update successfully", success: true, hallId: hall.id})
        } catch (err) {
            setState({"hallPriceUpdate": "Failed to update.", success: false, hallId: hall.id})
        }
    }
    const removeHallPrice = async (e, id) => {
        ...
        try {
            const api = await authApi().delete(endpoints['hall-price-delete'](id), {method: "DELETE", headers: {"Authorization": cookie.load("token").access_token}})
            setState({"hallPriceDelete": "Delete successfully", success: true, hallId: hall.id})
        } catch (err) {
            setState({"hallPriceDelete": "Failed to delete.", success: false, hallId: hall.id})
        }
    }

    return <>
         {hall.prices.length > 0 ? <>
            <label for={"hallPriceChoose-HalId-" +  hall.id}>Choose price to modify: </label>
            <select name={"hallPriceChoose-HalId-" +  hall.id} className="mb-3 ms-2" defaultValue={hallPrice ? hallPrice : -1} onChange={(e) => {setHallPrice(e.target.value)}}>
            ...
            {hallPrice && hall.prices.filter(p => p.id == hallPrice).map((price) => {
                return <div className="m-2 p-2">
                    <form onSubmit={(e) => {updateHallPrice(e, price.id)}}>
                        ...
                        <input type="text" id="discount" name="discount" class="form-control" defaultValue={price.discount}/>
                        <button type="submit" className="btn btn-primary mt-3">Update this price</button>
                        <button onClick={(e) => {removeHallPrice(e, price.id)}} className="btn btn-danger mt-3 ms-2">Delete this price</button>
                        <button onClick={(e) => { e.preventDefault(); setHallPrice(-1)}} className="btn btn-secondary mt-3 ms-2">Close</button>
                    </form>
                </div>
            })}
    </>
