...
const EditBranch = (props) => {
  ...
  const branchId = props.id || params.get("id");
  ...
  }, [error, state, hallPrice, branchId]);
  ...
  const updateBranchInfo = async (e, id) => {
    ...
    try {
      await authApi().put(endpoints["branch-edit"](id), data);
  ...
  const updateHall = async (e, id) => {
    ...
    try {
      await authApi().put(endpoints["hall-update"](id), data);
      setState({ hall: "Success Update.", success: true, hallId: id });
    } catch (err) {
      setState({
        hall: "Error when updating hall.",
        success: false,
        hallId: id,
      });
    }
  ...
  const removeHall = async (e, id, hallName) => {
    e.preventDefault();
    try {
      await authApi().delete(endpoints["hall-delete"](id), {
        method: "DELETE",
        headers: { Authorization: cookie.load("token").access_token },
      });
      setState({
        deleteHall: 'Delete hall "' + hallName + '" successfully.',
        success: true,
      });
    } catch (err) {
      setState({
        deleteHall: 'Delete hall "' + hallName + '" is denied..',
        success: false,
      });
    }
  };
  const deactivateHall = async (e, id, isActive) => {
    ...
    try {
      await authApi().post(
        isActive
          ? endpoints["hall-deactivate"](id)
          : endpoints["hall-activate"](id)
      );
      setState({
        hallDeactivate: action + " successfull",
        success: true,
        hallId: id,
      });
    } catch (err) {
      setState({
        hallDeactivate: "Failed to " + action,
        success: false,
        hallId: id,
      });
    }
  ...
  return (
    <>
      <form
        ...
        onSubmit={(e) => updateBranchInfo(e, branchId)}
      >
        <h3 className="text-decoration-underline">Thong tin branch</h3>
        <span className="text-danger">{error.branch}</span>
        <span class={state.success ? "text-success" : "text-danger"}>
          {state.deleteHall}
        </span>
        {branch && (
          <>
            <p>
              Branch id: <span className="fw-bold">{branch.id}</span>
            </p>
            ...
            <input
              ...
              name="houseNumber"
              class="form-control"
              defaultValue={branch.houseNumber}
            />
            <label class="form-label">You want to disable this branch?</label>{" "}
            <br></br>
            ...
            <input
              type="radio"
              id="branch-disable-false"
              name="isActive"
              value={false}
              defaultChecked={!branch.isActive}
            />
            <label for="branch-disable-false"> No </label>
            <br></br>
      ...
      <div class="m-1 p-2 rounded border">
        <h3 className="text-decoration-underline">
          Danh sach cac hall trong branch
        </h3>
                  ...
                  >
                    <div className="d-flex justify-content-between">
                      ...
                      <div>
                        <button
                          onClick={(e) => {
                            deactivateHall(e, hall.id, hall.isActive);
                          }}
                          className={
                            "btn " +
                            (hall.isActive ? "btn-warning" : "btn-success")
                          }
                        >
                        ...
                        <button
                          onClick={(e) => {
                            removeHall(e, hall.id, hall.name);
                          }}
                    ...
                    />
                    <label for="tableCount" class="form-label">
                      Table Count
                    </label>
                    <input
                      type="text"
                    ...
                    <input
                      ...
                      name="guestUpTo"
                      class="form-control"
                      defaultValue={hall.guestUpTo}
                    />
                    <label class="form-label">
                      You want to disable this hall?
                    </label>{" "}
                    <br></br>
                    <label class="form-label">Enable</label> <br></br>
                    <input
                      ...
                      name="isActive"
                      value={true}
                      defaultChecked={hall.isActive}
                    />
                    <label for="wOnly-Wedding"> Yes </label>
                    <input
                      type="radio"
                      id="hall-disable-false"
                      name="isActive"
                      value={false}
                      defaultChecked={!hall.isActive}
                    />
                    ...
                    <span
                      class={state.success ? "text-success" : "text-danger"}
                    >
                      {state.hall && state.hallId === hall.id && state.hall}
                    </span>
                    {state.hallId === hall.id && (
                      <span
                        class={state.success ? "text-success" : "text-danger"}
                      >
                        {state.hallDeactivate}
                      </span>
                    )}
                  </form>
