  ...
  useEffect(() => {
    let api = async (pageSize, pageIndex) => {
      let { data } = await authApi().get(
        endpoints["wedding"] +
          (filter.allPaid ? "?allPaid=" + filter.allPaid : "") +
          (filter.completed ? "?completed=" + filter.completed : "") +
          (filter.onlyDeposit ? "?onlyDeposit=" + filter.onlyDeposit : "")
      );
      setOrders(data);
    };
    api(100, 1);
      ...
      <div className="d-flex justify-content-end me-2">
        ...
        <div>
          ...
          {/* <input type="checkbox" id='onlyDeposit' name="onlyDeposit" onChange={() => {(filter.onlyDeposit == null) ? setFilter({...filter, onlyDeposit: true}) : setFilter({...filter, onlyDeposit: false})}} />
            <label htmlFor="onlyDeposit" >Have Deposit?</label> */}
        </div>
      </div>
        ...
        <tbody>
          {orders != null &&
            orders
              ...
              .map((order, index) => (
                <tr
                  className={!order.receiptNo ? "table-active" : ""}
                  key={index}
                >
                  ...
                  <td>
                    {new Date(order.celebrityDate).toLocaleDateString("vn")}
                  </td>
                  ...
                  <td>
                    <div>
                      <Link
                        ...
                        to={"/admin/wedding/view?id=" + order.id}
                      >
                        <GrFormView /> View
                      </Link>
                      <Link
                        className="text-decoration-none"
                        to={"/admin/wedding/edit?id=" + order.id}
                      >
                        <AiFillEdit /> Edit
                      </Link>
