...
const Order = () => {
    ...
    useEffect(() => {
        let fromDate = (time.fromDate != null  ? "&fromDate=" + new Date(time.fromDate).toLocaleDateString('en-GB') : "")
        if (fromDate != null) fromDate = fromDate.replaceAll('/', "-")
        let toDate = (time.toDate != null ? "&toDate=" + new Date(time.toDate).toLocaleDateString('en-GB') : "");
        if (toDate != null) toDate = toDate.replaceAll('/', '-')
        let api = async (pageSize, pageIndex) => {
        let {data} = await authApi().get(endpoints['orders'] + "?pageIndex=" + pageIndex + "&pageSize=" + pageSize 
        + fromDate + toDate)
        setOrders(data)}
        api(100, 1)
    }, [kw, state, time])
    ...
    return <>
        <div className="d-flex justify-content-end me-2">
          ...
          <div> From Date: <input type="date" name="fromDate" onChange={(e) => {setTime({...time, fromDate: e.target.value})}} />
          To Date: <input type="date" name="toDate" onChange={(e) => {setTime({...time, toDate: e.target.value}); console.log(e)}} />
          </div>
        </div>
            ...
            <tr className={!order.receiptNo ? "table-active" : ""}>
              ...
              <td>
                <div>
                  ...
                  { ((new Date().getTime() - 5 * 60) < new Date(order.createdDate).getTime()) && 
