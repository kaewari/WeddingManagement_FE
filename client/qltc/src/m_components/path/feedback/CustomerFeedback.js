...
const CustomerFeedback = () => {
  ...
  const deleteFeedback = async (e, feedbackId) => {
    try {
      const api = await authApi().delete(
      ...
      setState({
        deleteFeedback: "Delete feedback successfully",
        success: true,
      });
    } catch (err) {
      ...
      <div className="m-1 p-2">
        ...
        <div className="d-flex justify-content-end">
          <input
            ...
            name="replied"
            onChange={(e) => {
              setKw({ ...kw, customerId: e.target.value });
            }}
            placeholder="id cua khach hang"
          ...
          <input
            ...
            onChange={() => {
              kw.replied == null
                ? setKw({ ...kw, replied: true })
                : setKw({ ...kw, replied: !kw.replied });
            }}
          />
          <label for="replied">
            {kw.replied != null
          ...
          <tbody>
            {feedbacks != null &&
              feedbacks
                ...
                .map((feedback, index) => (
                  <tr
                    className={feedback.reply ? "table-active" : ""}
                    key={index}
                  >
                    ...
                    <td>
                      <div>
                        <Link
                          ...
                          to={"/admin/feedback/view?id=" + feedback.id}
                        >
                          <GrFormView /> View
                        </Link>
                        <Link
